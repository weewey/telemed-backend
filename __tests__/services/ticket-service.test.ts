...
describe("TicketService", () => {
  ...
  describe("#create", () => {
    ...
    describe("Error scenarios", () => {
      describe("when there is no associated patient / queue / clinic id", () => {
        it("should throw 404 NotFound error ENTITY_NOT_FOUND", async () => {
          jest.spyOn(TicketRepository, "create")
            .mockRejectedValue(new RepositoryError(Errors.ENTITY_NOT_FOUND.code, Errors.ENTITY_NOT_FOUND.message));
      ...
      describe("when displayNumber is null", () => {
        it("should throw 404 NotFound error UNABLE_TO_CREATE_TICKET_AS_DISPLAY_NUM_IS_NULL", async () => {
          jest.spyOn(TicketRepository, "create")
            .mockRejectedValue(new RepositoryError(Errors.UNABLE_TO_CREATE_TICKET_AS_DISPLAY_NUM_IS_NULL.code,
              Errors.UNABLE_TO_CREATE_TICKET_AS_DISPLAY_NUM_IS_NULL.message));
          await expect(TicketService.create(ticketAttr))
            .rejects.toThrow(new NotFoundError(Errors.UNABLE_TO_CREATE_TICKET_AS_DISPLAY_NUM_IS_NULL.code,
              Errors.UNABLE_TO_CREATE_TICKET_AS_DISPLAY_NUM_IS_NULL.message));
        });
      });
