    ...
    describe('POST /tickets', function () {
        ...
        describe('Successful scenarios', () => {
            it("should return 201 with the expected body", async () => {
                jest.spyOn(QueueService, "getQueuesByClinicAndStatus").mockResolvedValue([{id:queueId, latestGeneratedTicketDisplayNumber:displayNumber} as any]);
        ...
        describe('Error scenarios', () => {
            it("should return 400 if queue not found", async () => {
                jest.spyOn(QueueService, "getQueuesByClinicAndStatus").mockResolvedValue([{id:2, latestGeneratedTicketDisplayNumber:displayNumber} as any]);
                jest.spyOn(TicketService, "create").mockResolvedValue(ticket);
                await request(app).post(ticketsPath)
                    .send({ patientId, queueId, clinicId })
